// Shared mock data for the Reliable Properties static site.
// Adapted from project/property-data.js (the Claude Design handoff bundle).
(function (global) {
  var PHONE_PRIMARY = '9890982473';
  var PHONE_SECONDARY = '8668611436';
  ...
  var amenities = [
    'Swimming Pool', 'Gym', 'Clubhouse', 'Garden',
    'Security', 'Parking', "Children's Play Area", 'Power Backup'
  ];

  function waLink(text) {
    return 'https://wa.me/91' + PHONE_PRIMARY + '?text=' + encodeURIComponent(text);
  }

  function findBySlug(slug) {
    for (var i = 0; i < properties.length; i++) {
      if (properties[i].slug === slug) return properties[i];
    }
    return null;
  }
